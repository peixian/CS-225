...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    // std::cout << "THE MATCHING ELEMENT VALUE IS " << subroot->elements[first_larger_idx].value << std::endl;
    // std::cout << "FIRST LARGER IDX IS " << first_larger_idx << std::endl;
    ...
    //check if the subroot is a leaf
    if (subroot->is_leaf) {
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
	...
	//copies elements and children to the right
    for (size_t i = mid_elem_idx+1; i<new_left->elements.size(); i++) {
        new_right->elements.push_back(new_left->elements[i]);
    }
    for (size_t i = mid_child_idx; i<new_left->children.size(); i++) {
        new_right->children.push_back(new_left->children[i]);
    }

    //clears the elements and children of the left
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	if(subroot->is_leaf) {
        //check if the key already exists in the elements array
		if (first_larger_idx >= subroot->elements.size() || subroot->elements[first_larger_idx].key != pair.key) {
			auto first_larger_itr = subroot->elements.begin() + first_larger_idx;
			subroot->elements.insert(first_larger_itr, pair);
		}
		return;
	}
	//figure out which child to call the recursive function on
...
template <class K, class V>
void BTree<K, V>::printTree(const BTreeNode* subroot, int indent) {

    std::cout << std::string(indent, ' ') << *subroot << std::endl;
    if (subroot->is_leaf) {
        return;
    }
    else {
        for (size_t i = 0; i<subroot->children.size(); i++) {
            printTree(subroot->children[i], indent+4);
        }
        std::cout<< std::endl;
    }
}